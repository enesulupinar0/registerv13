...
module.exports = {
    ...
    stg: async(client, message, args, config, Roles, Channels, Permissions, Emojis) => {
        ...
        await member.roles.add(Roles.WomanRoles).catch(err => {{}})
        ...
        Database.find({sunucu: message.guild.id, user: member.id}, async(err, res) => {
            if(res.length < 1) {
                ...
                message.channel.send({embeds: [
                    ...
                    .setDescription(`${member} kişisi **Kadın** olarak kayıt edildi, ismi \`${userName}\` olarak değiştirildi.`)
                    .setColor('RANDOM')
                ]})
                ...
                new Database({
                    ...
                    isim: Name,
                    yaş: Age,
                    rolleri: `<@&${Roles.WomanRole}>`, 
                    sonİsimYaş: userName,
                    tarih: Date.now()
                }).save().catch(err => console.log(`Kadın üye datası kayıt edilirken sorun yaşandı!\nSorun:`+ err))
                return;
            }
            ...
            .addField(`- Verilen Roller`, `${Roles.WomanRoles.map(x => `<@&${x}>`)}`, false)
            .addField(`- Alınan Roller`, `${Roles.UnregisterRoles.map(x => `<@&${x}>`)}`, false)
            .addField(`- Cinsiyet`, `Kadın`, false)
            .setColor('RANDOM')
